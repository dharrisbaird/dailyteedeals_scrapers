...
class YeteeCommon(scrapy.Spider):
    ...
    def parse_product_page(self, response):
        ...
        loader.add_xpath('artist_name', '//div[@class="byline"]/a/text()')
        loader.add_xpath('image_url', '//div[@class="slide"][1]/img/@src')
        loader.add_xpath('prices', '//div[@class="price"]/text()')

        artist_url = urlparse.urljoin(response.url, response.xpath('//div[@class="byline"]/a/@href').extract_first())
    ...
    def parse_artist_page(self, response):
        ...
        loader.add_xpath('artist_urls', '//div[@class="artist-social"]//a/@href')
...
class YeteeDealSpider(YeteeCommon):
    ...
    def parse(self, response):
        for product_url in response.xpath('//div[@class="featured-tee"]//a[contains(@href, "detail")]/@href').extract():
